...
exports.SELF_FIX_SYSTEM = exports.STEP_GENERATE_SYSTEM = exports.STEP_EDIT_SYSTEM = exports.PLAN_SYSTEM = exports.REVIEW_SYSTEM = exports.GENERATE_SYSTEM = exports.EDIT_SYSTEM = exports.EXPLAIN_SYSTEM = exports.CHAT_SYSTEM = exports.INTENT_SYSTEM = void 0;
...
exports.buildSelfFixPrompt = buildSelfFixPrompt;
// ── Intent Classification ────────────────────────────────────────────
exports.INTENT_SYSTEM = `You are an intent classifier for a coding assistant. Analyze the user's message and determine the appropriate action.
...
function buildIntentPrompt(userMessage, activeFile, hasSelection) {
...
exports.CHAT_SYSTEM = `You are Agent Code — a helpful AI coding assistant running locally.
...
// ── Explain ──────────────────────────────────────────────────────────
exports.EXPLAIN_SYSTEM = `You are a code explainer. Provide clear, structured explanations using markdown.
...
function buildExplainPrompt(code, lang, instruction) {
    return `${instruction}\n\nLanguage: ${lang}\n\n\`\`\`${lang}\n${code}\n\`\`\``;
}
// ── Edit ─────────────────────────────────────────────────────────────
exports.EDIT_SYSTEM = `You are an expert code editor. Follow these rules strictly:
...
function buildEditPrompt(code, instruction, fileName, lang) {
...
// ── Generate ─────────────────────────────────────────────────────────
exports.GENERATE_SYSTEM = `You are a code generator. Follow these rules:
...
function buildGeneratePrompt(instruction, context) {
...
// ── Review ───────────────────────────────────────────────────────────
exports.REVIEW_SYSTEM = `You are a code reviewer. Analyze code and report issues with this format:
...
IMPORTANT: Respond in the same language as the user's message.`;
function buildReviewPrompt(code, lang, instruction) {
    return `${instruction}\n\nLanguage: ${lang}\n\n\`\`\`${lang}\n${code}\n\`\`\``;
}
// ── Planning (Multi-step) ────────────────────────────────────────────
exports.PLAN_SYSTEM = `You are a software architect and planner. Given a task description and project context, create a step-by-step plan.
...
function buildPlanPrompt(instruction, projectTree, context) {
...
// ── Step Execution ───────────────────────────────────────────────────
exports.STEP_EDIT_SYSTEM = `You are executing a step in a multi-step coding plan.
...
exports.STEP_GENERATE_SYSTEM = `You are executing a step in a multi-step coding plan.
...
// ── Self-Fix ─────────────────────────────────────────────────────────
exports.SELF_FIX_SYSTEM = `You are a debugging assistant. Given code that caused a build/lint error, fix the error.
Return ONLY the fixed code — no explanations, no markdown fences.
Fix ONLY the error — do not change anything else.`;
function buildSelfFixPrompt(code, error, fileName) {
